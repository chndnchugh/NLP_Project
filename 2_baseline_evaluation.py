...
def run_baseline(model, tokenizer, dataset: DatasetDict):
    ...
    test_texts  = df["Headline"].tolist()[-len(true_labels):]  # approximate match
...
def report(true_labels, pred_labels, title="Baseline (zero-shot)"):
    ...
    plt.savefig(f"results/{fname}_confusion.png", dpi=150)
    plt.close()
    print(f"Confusion matrix saved to results/{fname}_confusion.png")
...
if __name__ == "__main__":
    ...
    np.save("results/baseline_preds.npy", pred_labels)
